...
import {
  ConnectionString,
  SharedAccessSignature as CommonSaS,
...
export class MockDevice {
  ...
  private iotHubDevice: IoTHubDevice = {
    client: undefined,
    hubName: undefined,
  };
  ...
  initialize(device: any) {
    ...
    this.RESTART_LOOP =
      Utils.getRandomNumberBetweenRange(min, max, true) * 3600000;
  ...
  reconfigDeviceDynamically(valueOnlyUpdate: boolean) {
    ...
    this.log(
      "DEVICE/MODULE HAS BEEN CONFIGURED",
      LOGGING_TAGS.CTRL.DEV,
      LOGGING_TAGS.LOG.OPS
    );
    ...
    if (this.device.configuration.planMode) {
      ...
      this.device.plan.startup.forEach((item) => {
        const comm = this.device.comms[
          this.resolversCollection.idPropToCommIndex[item.property]
        ];
        if (comm.sdk === "twin") {
          this.twinRLProps.push(comm);
          this.twinRLPropsPlanValues.push(item.value);
          this.twinRLReportedTimers.push({
            timeRemain: config["startDelay"],
            originalTime: config["startDelay"],
          });
        } else if (comm.sdk === "msg") {
          this.msgRLProps.push(comm);
          this.msgRLPropsPlanValues.push(item.value);
          this.msgRLReportedTimers.push({
            timeRemain: config["startDelay"],
            originalTime: config["startDelay"],
          });
        }
      ...
      this.device.plan.timeline.forEach((item) => {
        ...
        if (comm.sdk === "twin") {
          ...
          this.twinRLPropsPlanValues.push(item.value);
          this.twinRLReportedTimers.push({
            timeRemain: this.planModeLastEventTime,
            originalTime: this.planModeLastEventTime,
          });
        } else if (comm.sdk === "msg") {
          this.msgRLProps.push(comm);
          this.msgRLPropsPlanValues.push(item.value);
    ...
    for (const i in this.device.comms) {
      ...
      if (this.device.comms[i]._type != "property") {
        continue;
      }
      ...
      if (comm.runloop && comm.runloop.include === true) {
        ...
        if (!comm.runloop._ms) {
          if (!comm.runloop.valueMax) {
            comm.runloop.valueMax = comm.runloop.value;
          }

          let newRunloopValue = 0;
          if (comm.runloop.override) {
            const simLoop =
              comm.runloop.unit === "secs" ? this.LOOP_SECS : this.LOOP_MINS;
            newRunloopValue = Utils.getRandomNumberBetweenRange(
              simLoop.min,
              simLoop.max,
              true
            );
            ...
            newRunloopValue = Utils.getRandomNumberBetweenRange(
              comm.runloop.value,
              comm.runloop.valueMax,
              true
            );
          }
          comm.runloop._ms =
            newRunloopValue * (comm.runloop.unit === "secs" ? 1000 : 60000);
        }
        ...
        if (comm.mock) {
          ...
          if (comm.mock._type != "function") {
            ...
            startValue =
              comm.mock.running && comm.mock.running > 0
                ? comm.mock.running
                : comm.mock.init;
          } else {
          ...
          mockSensorTimerObject = {
            sliceMs: slice,
            remainingMs: comm.mock.timeToRunning,
          };
        ...
        if (comm.sdk === "twin") {
          this.twinRLProps.push(comm);
          this.twinRLReportedTimers.push({
            timeRemain: comm.runloop._ms,
            originalTime: comm.runloop._ms,
          });
          if (mockSensorTimerObject != null) {
            this.twinRLMockSensorTimers[comm._id] = mockSensorTimerObject;
          }
        ...
        if (comm.sdk === "msg") {
          ...
          if (mockSensorTimerObject != null) {
            this.msgRLMockSensorTimers[comm._id] = mockSensorTimerObject;
          }
          if (comm.runloop.onStartUp) {
    ...
    this.firstSendMins = (
      Math.min(
        Math.min.apply(
          null,
          this.twinRLReportedTimers.map((v) => v.timeRemain)
        ),
        Math.min.apply(
          null,
          this.msgRLReportedTimers.map((v) => v.timeRemain)
        )
      ) / 60000
    ).toFixed(1);
  }
  ...
  buildIndexes() {
    this.device.comms.forEach((comm, index) => {
      ...
      if (comm._type === "method") {
        ...
        const key =
          comm.component && comm.component.enabled
            ? `${comm.component.name}*${comm.name}`
            : comm.name;
  ...
  processMockDevicesCMD(name: string) {
    ...
    if (methodName === this.CMD_SHUTDOWN) {
      this.log(
        "DEVICE/MODULE METHOD SHUTDOWN ... STOPPING IMMEDIATELY",
    ...
    if (methodName === this.CMD_REBOOT) {
      this.log(
        "DEVICE/MODULE METHOD REBOOT ... RESTARTING IMMEDIATELY",
        LOGGING_TAGS.CTRL.HUB,
        LOGGING_TAGS.LOG.OPS
      );
      this.stop();
  ...
  start(delay) {
    ...
    if (delay) {
      this.log(
        `DEVICE/MODULE DELAYED START SECONDS: ${delay / 1000}`,
        LOGGING_TAGS.CTRL.HUB,
        LOGGING_TAGS.LOG.OPS
      );
      this.logCP(
  ...
  async startDevice() {
    ...
    if (this.device.configuration._kind === "module") {
      ...
      this.logCP(
        LOGGING_TAGS.CTRL.MOD,
        LOGGING_TAGS.LOG.OPS,
        LOGGING_TAGS.LOG.EV.TRYING
      );

      const { deviceId, moduleId } = Utils.decodeModuleKey(this.device._id);
      if (this.device.configuration.environmentModule) {
        if (
          !GLOBAL_CONTEXT.IOTEDGE_WORKLOADURI &&
          !GLOBAL_CONTEXT.IOTEDGE_DEVICEID &&
          !GLOBAL_CONTEXT.IOTEDGE_MODULEID &&
          !GLOBAL_CONTEXT.IOTEDGE_MODULEGENERATIONID &&
          !GLOBAL_CONTEXT.IOTEDGE_IOTHUBHOSTNAME &&
          !GLOBAL_CONTEXT.IOTEDGE_AUTHSCHEME
        ) {
          this.log(
            `MODULE '${moduleId}' ENVIRONMENT CHECK FAILED - MISSING IOTEDGE_*`,
            LOGGING_TAGS.CTRL.MOD,
            LOGGING_TAGS.LOG.OPS
          );
          this.log(
        ...
        if (
          GLOBAL_CONTEXT.IOTEDGE_DEVICEID != deviceId ||
          GLOBAL_CONTEXT.IOTEDGE_MODULEID != moduleId
        ) {
          this.log(
            `MODULE '${moduleId}' DOES NOT MATCH THE MANIFEST CONFIGURATION FOR HOST DEVICE/MODULE (NOT A FAILURE)`,
            LOGGING_TAGS.CTRL.MOD,
            LOGGING_TAGS.LOG.OPS
          );
          this.log(
        ...
        try {
          this.iotHubDevice.client = await ModuleClient.fromEnvironment(
            Protocol
          );
          this.log(
            `MODULE '${moduleId}' CHECK PASSED. ENTERING MAIN LOOP`,
            LOGGING_TAGS.CTRL.MOD,
            LOGGING_TAGS.LOG.OPS
          );
          this.logCP(
            LOGGING_TAGS.CTRL.MOD,
            LOGGING_TAGS.LOG.OPS,
            LOGGING_TAGS.LOG.EV.CONNECTED
          );
          this.mainLoop();
        } catch (err) {
          this.log(
            `MODULE '${moduleId}' FAILED TO CONNECT THROUGH ENVIRONMENT TO IOT HUB: ${err}`,
            LOGGING_TAGS.CTRL.MOD,
            LOGGING_TAGS.LOG.OPS
          );
          this.log(
            `MODULE '${moduleId}' WILL SHUTDOWN`,
            LOGGING_TAGS.CTRL.DEV,
            LOGGING_TAGS.LOG.OPS
          );
          this.logCP(
    ...
    if (
      this.device.configuration._kind === "dps" ||
      this.device.configuration._kind === "edge" ||
  ...
  /**
   * Edge module status are taken from the $edgeAgent twin.
  ...
  stop() {
    if (this.delayStartTimer) {
      ...
      this.delayStartTimer = null;
      this.log(
        `DEVICE/MODULE DELAYED START CANCELED`,
        LOGGING_TAGS.CTRL.DEV,
        LOGGING_TAGS.LOG.OPS
      );
      this.logCP(
  ...
  cleanUp() {
    ...
    try {
      ...
      this.log(
        `DEVICE/MODULE CLIENT TEARDOWN ERROR: ${err.message}`,
        LOGGING_TAGS.CTRL.DEV,
        LOGGING_TAGS.LOG.OPS
      );
      this.logStat(LOGGING_TAGS.STAT.ERRORS);
  ...
  mainLoop() {
    ...
    try {
      this.iotHubDevice.client.open(async () => {
        ...
        for (const i in this.device.comms) {
          if (
            this.device.comms[i].runloop &&
            this.device.comms[i].runloop.onStartUp &&
            !this.device.comms[i].runloop.include
          ) {
        ...
        this.iotHubDevice.client.getTwin((err, twin) => {
          if (err) {
            this.log(
              "IOT HUB TWIN REQUEST FAILED. CLIENT IN BAD STATE",
              LOGGING_TAGS.CTRL.HUB,
              LOGGING_TAGS.LOG.OPS
            );
            this.stop();
          ...
          twin.on("properties.desired", (delta) => {
            ...
            this.log(
              ...
              LOGGING_TAGS.CTRL.HUB,
              LOGGING_TAGS.MSG.TWIN,
              LOGGING_TAGS.DATA.RECV
            );
          ...
          if (this.device.comms.length === 0) {
            this.log(
              `DEVICE/MODULE HAS NO CAPABILITIES DEFINED. DEVICE/MODULE CAN RECEIVE EVENTS BUT WILL NOT SEND ANY DATA`,
              LOGGING_TAGS.CTRL.HUB,
              LOGGING_TAGS.LOG.OPS
            );
          } else {
            if (Utils.isNumeric(this.firstSendMins)) {
              this.log(
                `LOOPS HAVE BEEN DEFINED. DATA WILL START SENDING IN ${this.firstSendMins} MINUTES`,
                LOGGING_TAGS.CTRL.HUB,
                LOGGING_TAGS.LOG.OPS
              );
            } else {
              this.log(
                `NO LOOPS HAVE BEEN DEFINED. SEND DATA MANUALLY`,
                LOGGING_TAGS.CTRL.HUB,
                LOGGING_TAGS.LOG.OPS
              );
            }
          ...
          this.methodRLTimer = setInterval(() => {
            if (this.methodReturnPayload != null) {
              twin.properties.reported.update(
                this.methodReturnPayload,
                (err) => {
                  this.log(
                    err
                      ? err.toString()
                      : JSON.stringify(this.methodReturnPayload),
                    LOGGING_TAGS.CTRL.HUB,
                    LOGGING_TAGS.MSG.TWIN,
          ...
          this.twinRLTimer = setInterval(() => {
            let payload: ValueByIdPayload = <ValueByIdPayload>(
              this.calcPropertyValues(
                this.twinRLProps,
                this.twinRLReportedTimers,
                this.twinRLMockSensorTimers,
                this.twinRLPropsPlanValues,
      ...
      this.log(
        `SDK OPEN ERROR (CHECK CONN STRING): ${err.message}`,
        LOGGING_TAGS.CTRL.HUB,
        LOGGING_TAGS.LOG.OPS
      );
      this.logCP(
  ...
  sendPropertyResponse(sentValue, name, version, component) {
    ...
    const property: Property = this.device.comms[
      this.resolversCollection.nameDesiredToIndex[name]
    ];
    if (!property) {
      return;
    }
    ...
    if (property.asProperty && property.asPropertyId) {
      ...
      const p = this.device.comms.filter((x) => {
        return x._id === property.asPropertyId;
      });
      // should only be 1
      for (const send in p) {
        let json: ValueByIdPayload = <ValueByIdPayload>{};
  ...
  sendPlanResponse(propertyId) {
    ...
    const property = this.device.plan.receive.find((prop) => {
      return prop.property === propertyId;
    });
    if (property) {
      const outboundProperty: Property = this.device.comms[
        this.resolversCollection.idPropToCommIndex[property.propertyOut]
      ];
      if (!outboundProperty) {
        return;
      }
      const payload = <ValueByIdPayload>{
        [outboundProperty._id]: property.value,
      };
      if (outboundProperty.sdk === "twin") {
        this.updateTwin(payload);
      } else {
  ...
  sendMethodResponse(method: Method) {
    if (this.device.configuration.planMode) {
      this.sendPlanResponse(
        this.resolversCollection.nameMethodToId[method.name]
      );
    } else if (
      !this.device.configuration.planMode &&
      method.asProperty &&
      method.asPropertyId
    ) {
      // set up the global desired override
      ...
      const p = this.device.comms.filter((x) => {
        return x._id === method.asPropertyId;
      });
      // should only be 1
      for (const send in p) {
        let json: ValueByIdPayload = <ValueByIdPayload>{};
      ...
      this.methodReturnPayload = Object.assign(
        {},
  ...
  registerC2D() {
    this.iotHubDevice.client.on("message", (msg) => {
      ...
      try {
        ...
        const cloudMethod =
          cloudNameParts.length === 2 ? cloudNameParts[1] : cloudNameParts[0];
        ...
        if (this.resolversCollection.nameC2dToCommIndex[cloudMethod]) {
          const method: Method = this.device.comms[
            this.resolversCollection.nameC2dToCommIndex[cloudMethod]
          ];
          this.log(
            cloudMethod + " " + JSON.stringify(cloudMethodPayload),
            LOGGING_TAGS.CTRL.HUB,
            LOGGING_TAGS.DATA.METH,
            LOGGING_TAGS.DATA.RECV,
            "C2D REQUEST AND PAYLOAD"
          );
          this.logStat(LOGGING_TAGS.STAT.C2D);
          Object.assign(this.receivedMethodParams, {
            [method._id]: {
              date: new Date().toUTCString(),
              payload: JSON.stringify(cloudMethodPayload, null, 2),
            },
          });
        ...
        this.log(
          ...
          LOGGING_TAGS.DATA.METH,
          LOGGING_TAGS.DATA.SEND,
          "C2D ERROR PARSING MESSAGE BODY"
        );
      }

      this.iotHubDevice.client.complete(msg, (err) => {
        this.log(
          `${err ? err.toString() : error ? "FAILED" : "SUCCESS"}`,
          LOGGING_TAGS.CTRL.HUB,
          LOGGING_TAGS.LOG.OPS,
          null,
          "C2D COMPLETE"
  ...
  registerDirectMethods() {
    for (const key in this.resolversCollection.nameDmToCommIndex) {
      const clientMethodKey =
        this.device.configuration._kind === "module"
          ? "onMethod"
          : "onDeviceMethod";
      this.iotHubDevice.client[clientMethodKey](key, (request, response) => {
        const method: Method = this.device.comms[
          this.resolversCollection.nameDmToCommIndex[key]
        ];
        ...
        this.log(
          ...
          LOGGING_TAGS.DATA.METH,
          LOGGING_TAGS.DATA.RECV,
          "DIRECT METHOD REQUEST AND PAYLOAD"
        );
        Object.assign(this.receivedMethodParams, {
          [method._id]: {
            date: new Date().toUTCString(),
        ...
        response.send(parseInt(method.status), methodPayload, (err) => {
          this.log(
            ...
            LOGGING_TAGS.DATA.METH,
            LOGGING_TAGS.DATA.SEND,
            "DIRECT METHOD RESPONSE PAYLOAD"
          );
          this.logStat(LOGGING_TAGS.STAT.COMMANDS);
          this.messageService.sendAsLiveUpdate(this.device._id, {
            [method._id]: new Date().toUTCString(),
          });
  ...
  connectClient(connectionString) {
    ...
    if (this.useSasMode) {
      ...
      let sas: any = SharedAccessSignature.create(
        cn.HostName,
        cn.DeviceId,
        cn.SharedAccessKey,
        this.sasTokenExpiry
      ...
      const trueHours = Math.ceil(
        (this.sasTokenExpiry - Math.round(Date.now() / 1000)) / 3600
      );
      this.log(
        `CONNECTING VIA SAS.TOKEN EXPIRES AFTER ${trueHours} HOURS`,
        LOGGING_TAGS.CTRL.HUB,
        LOGGING_TAGS.LOG.OPS
      );
    } else {
      ...
      // store hub name
      this.iotHubDevice.hubName = ConnectionString.parse(
        connectionString
      ).HostName;
      this.log(
        `CONNECTING VIA CONN STRING`,
        LOGGING_TAGS.CTRL.HUB,
        LOGGING_TAGS.LOG.OPS
      );
    }
    ...
    this.log(
      `DEVICE/MODULE AUTO RESTARTS EVERY ${this.RESTART_LOOP / 60000} MINUTES`,
      LOGGING_TAGS.CTRL.HUB,
      LOGGING_TAGS.LOG.OPS
    );

    // if edge, start all its modules too
    if (this.device.configuration._kind === "edge") {
    }
  }

  dpsRegistration() {
    if (this.registrationConnectionString === "init") {
      return;
    }
    if (this.dpsRetires === 0) {
      return;
    ...
    let transformedSasKey = config.isMasterKey
      ? this.computeDrivedSymmetricKey(config.sasKey, config.deviceId)
      : config.sasKey;
    ...
    let provisioningSecurityClient = new SymmetricKeySecurityClient(
      config.deviceId,
      transformedSasKey
    );
    ...
    provisioningClient.register((err: any, result) => {
      if (err) {
        let msg =
          (err.result &&
            err.result.registrationState &&
            err.result.registrationState.errorMessage) ||
          err;
        this.log(
          `REGISTRATION ERROR ${this.dpsRetires}: ${err}`,
          LOGGING_TAGS.CTRL.DPS,
          LOGGING_TAGS.LOG.OPS
        );
        this.logCP(
      ...
      this.registrationConnectionString =
        "HostName=" +
        result.assignedHub +
        ";DeviceId=" +
        result.deviceId +
        ...
        for (const c in transformed.package) {
          ...
          setTimeout(() => {
            twin.properties.reported.update(data, (err) => {
              ...
              this.messageService.sendAsLiveUpdate(
                this.device._id,
                transformed.live
              );
            });
  ...
  calcPropertyValues(
    runloopProperties: any,
    runloopTimers: any,
    propertySensorTimers: any,
    runloopPropertiesValues: any,
    startUpList: Array<string>
  ) {
    ...
    for (const i in runloopProperties) {
      ...
      let possibleResetTime =
        runloopTimers[runloopTimers.length - 1].timeRemain + originalTime;
      let res = this.processCountdown(
        p,
        timeRemain,
        possibleResetTime,
        startUpList
  ...
  async updateSensorValue(
    ...
    if (p.mock._type === "hotplate") {
      if (
        p.mock.reset &&
        Utils.isNumeric(p.mock.reset) &&
        Utils.formatValue(false, p.mock.reset) === p.mock._value
      ) {
        p.mock._value = Utils.formatValue(false, p.mock.init);
      } else {
        var newCurrent = p.mock._value + (slice - slice * p.mock.variance);
        p.mock._value =
          newCurrent <= p.mock.running ? newCurrent : p.mock.running;
      }
    }
    ...
    if (p.mock._type === "random") {
      p.mock._value = Math.floor(
        Math.random() * Math.floor(Math.pow(10, p.mock.variance) + 1)
      );
    }

    if (p.mock._type === "function" && process) {
      const res: any = await this.getFunctionPost(
        p.mock.function,
        p.mock._value
      );
      p.mock._value = res;
    ...
    if (p.mock._type === "inc" && process) {
      const inc =
        p.mock.variance && Utils.isNumeric(p.mock.variance)
          ? Utils.formatValue(false, p.mock.variance)
          : 1;
      if (
        p.mock.reset &&
        Utils.isNumeric(p.mock.reset) &&
      ...
      try {
        request.post(
          ...
          (err, response, body) => {
            if (err) {
              this.log(
                `FUNCTION ERROR: ${err.toString()}`,
                LOGGING_TAGS.DATA.FNC,
                "",
                LOGGING_TAGS.DATA.SEND
              );
              reject(err);
  ...
  transformPayload(payload: any) {
    ...
    for (const item in payload) {
      ...
      if (p && p.propertyObject) {
        ...
        var component =
          p.component && p.component.enabled ? p.component.name : null;
        if (component && !remap.package[component]) {
          remap.package[component] = {};
        }
        if (!component && !remap.package["_root"]) {
          remap.package["_root"] = {};
        }

        if (this.plugIn) {
          const val =
            p.propertyObject.type === "templated"
              ? JSON.parse(p.propertyObject.template)
              : payload[p._id];
          const res = this.plugIn.propertyResponse(
            this.device.configuration.deviceId,
            p,
            val
          );
        ...
        if (this.desiredOverrides[p._id]) {
          ...
          try {
            ...
            if (
              sendOverrides.convention &&
              typeof sendOverrides.value === "object" &&
              sendOverrides.value !== null
            ) {
              Object.assign(override, sendOverrides.value);
            }
          ...
          try {
            var replacement = p.propertyObject.template.replace(
              new RegExp(/\"AUTO_VALUE\"/, "g"),
              val
            );
  ...
  resolveAuto(macro: string, parameters?: any) {
    if (macro === "AUTO_STRING") {
      ...
      return Utils.getRandomValue(
        "integer",
        this.ranges["AUTO_INTEGER"]["min"],
        this.ranges["AUTO_INTEGER"]["max"]
      );
    } else if (macro === "AUTO_DOUBLE" || macro === "AUTO_FLOAT") {
      return Utils.getRandomValue(
        "double",
        this.ranges["AUTO_DOUBLE"]["min"],
        this.ranges["AUTO_DOUBLE"]["max"]
      );
      ...
      return Utils.getRandomVector(
        this.ranges["AUTO_VECTOR"]["min"],
        this.ranges["AUTO_VECTOR"]["max"]
      );
  ...
  logCP(type, operation, event) {
    this.messageService.sendAsControlPlane({
      [this.device._id]: [type, operation, event],
    });
  }
  ...
  logStat(type) {
    ...
    if (update === "MSG" && !this.stats.MSG_FIRST) {
      this.stats.MSG_FIRST = date;
    }
    if (update === "TWIN" && !this.stats.TWIN_FIRST) {
      this.stats.TWIN_FIRST = date;
    }

    if (update === "MSG") {
      this.stats.MSG_LAST = date;
    }
    if (update === "TWIN") {
      this.stats.TWIN_LAST = date;
    }
    ...
    if (this.stats.MSG_FIRST && this.stats.MSG_LAST && update === "MSG") {
      ...
      this.stats[LOGGING_TAGS.STAT.MSG.RATE] = (
        this.stats[type] / Math.round((f - s) / 60000)
      ).toFixed(2);
    }
    ...
    if (this.stats.TWIN_FIRST && this.stats.TWIN_LAST && update === "TWIN") {
      ...
      this.stats[LOGGING_TAGS.STAT.TWIN.RATE] = (
        this.stats[type] / Math.round((f - s) / 60000)
      ).toFixed(2);
    }
