...
export class DeviceStore {
  ...
  public deleteDevice = (d: Device) => {
    if (d.configuration.modules) {
      for (const index in d.configuration.modules) {
        ...
        this.messageService.removeStatsOrControl(
          d.configuration.modules[index]
        );
      }
    }
  ...
  public removeDeviceModule = (d: Device, moduleId: string) => {
    let i = d.configuration.modules
      ? d.configuration.modules.indexOf(moduleId)
      : -1;
    if (i > -1) {
      const payload = {
    ...
    i = d.configuration.edgeDevices
      ? d.configuration.edgeDevices.indexOf(moduleId)
      : -1;
    if (i > -1) {
      const payload = {
  ...
  public addDevice = (d: Device) => {
    ...
    if (d.configuration.mockDeviceCloneId) {
      this.cloneDeviceCommsAndPlan(d, d.configuration.mockDeviceCloneId);
    }
      ...
      for (let key in p) {
        if (d.comms[index].name === key) {
          if (d.comms[index].propertyObject.type === "templated") {
            d.comms[index].propertyObject["template"] = p[key]
  ...
  public updateDeviceProperty = (
    ...
    if (index > -1) {
      ...
      let converted = Utils.formatValue(
        d.comms[index].string,
        d.comms[index].value
      );
      ...
      if (sendValue === true && d.comms[index].sdk === "twin") {
        rd.updateTwin(json);
      }
      if (sendValue === true && d.comms[index].sdk === "msg") {
        rd.updateMsg(json);
      }
    }
  };
  ...
  public cleanPlan = (device: Device, propertyId: string) => {
    const newPlan: any = {};
    newPlan.startup = device.plan.startup.filter((p) => {
      return p.property != propertyId;
    });
    newPlan.timeline = device.plan.timeline.filter((p) => {
      return p.property != propertyId;
    });
    newPlan.receive = device.plan.receive.filter((p) => {
      return p.propertyOut != propertyId;
    });
    newPlan.random = device.plan.random.slice();
    device.plan = newPlan;
  };

  public updateDeviceMethod = (
    id: string,
    propertyId: string,
    payload: Property
  ) => {
  ...
  public startAll = () => {
    ...
    let to: number = 10;
    const batch =
      this.bulkRun != null && this.bulkRun["mode"]
        ? this.bulkRun["mode"]["batch"]["size"]
        : 10;
    ...
    let timer = setInterval(
      () => {
        ...
        if (to <= devices.length) {
          this.startAllBatch(from, to, devices);
        }
      },
  ...
  public startAllBatch = (from: number, to: number, devices: Array<Device>) => {
    for (let index = from; index < to; index++) {
      this.startDevice(devices[index]);
    }
  };
  ...
  public reapplyTemplate(
    ...
    for (let index in devices) {
      ...
      if (rd) {
        rd.updateDevice(devices[index], false);
      }
    }
  }
  ...
  public getCommonCapabilities(deviceList: Array<string>, applyAll: boolean) {
    ...
    for (const i in devices) {
      const deviceId = applyAll
        ? devices[i].configuration.deviceId
        : devices[i];
    ...
    for (const i in devices) {
      ...
      for (let index in device.comms) {
        ...
        if (allCapabilities || capabilitiesList.indexOf(comm.name) > -1) {
          // common to all
          if (payload.include.component) {
            comm.component = {
              enabled: payload.data.component ? true : false,
              name: payload.data.component || "",
            };
          }
          if (payload.include.geo) {
            device.configuration.geo = parseInt(payload.data.geo || 0);
          }
          ...
          if (comm["_type"] === "property" && comm.type.direction === "d2c") {
            ...
            if (
              payload.include.loop ||
              (comm.runloop && !comm.runloop.override)
            ) {
              comm.runloop = {
                include: true,
            ...
            if (payload.include.value) {
              if (!payload.data.value) {
                ...
                (payload.data.value.startsWith("[") &&
                  payload.data.value.endsWith("]")) ||
                (payload.data.value.startsWith("{") &&
                  payload.data.value.endsWith("}"))
              ) {
      ...
      if (rd) {
        rd.updateDevice(device, false);
      }
    }
  }

  private cloneDeviceCommsAndPlan(device: Device, cloneId: string) {
    const origDevice: Device = JSON.parse(
      JSON.stringify(this.store.getItem(cloneId))
    );
    if (Object.keys(origDevice).length != 0) {
      device.configuration.capabilityUrn =
        origDevice.configuration.capabilityUrn;
      ...
      for (let p of origDevice.comms) {
        ...
        for (const property in origDevice.plan.startup) {
          if (origDevice.plan.startup[property].property === origPropertyId) {
            origDevice.plan.startup[property].property = newPropertyId;
          }
        }

        for (const property in origDevice.plan.timeline) {
          if (origDevice.plan.timeline[property].property === origPropertyId) {
            origDevice.plan.timeline[property].property = newPropertyId;
          }
        }

        for (const property in origDevice.plan.random) {
          if (origDevice.plan.random[property].property === origPropertyId) {
            origDevice.plan.random[property].property = newPropertyId;
          }
        }

        for (const property in origDevice.plan.receive) {
          if (origDevice.plan.receive[property].property === origPropertyId) {
            origDevice.plan.receive[property].property = newPropertyId;
          }
          if (
            origDevice.plan.receive[property].propertyOut === origPropertyId
          ) {
            origDevice.plan.receive[property].propertyOut = newPropertyId;
          }
        }
      }
      ...
      for (let p of origDevice.comms) {
        if (cache[p.asPropertyId]) {
          p.asPropertyId = cache[p.asPropertyId];
        }
      }
